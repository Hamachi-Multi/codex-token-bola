...
def classify(prompt: str, cwd: Any) -> tuple[str, str]:
    ...
    if "duni audit" in text and ("duni mend" in text or "duni slice" in text or "next task" in text):
    ...
    if any(keyword in text for keyword in ("implement", "implementation", "fix", "modify", "create", "proceed with the task")):
        return "implementation", f"{project}_implementation"
    if any(keyword in text for keyword in ("check", "verify", "investigate", "research", "analyze", "analysis")):
        return "investigation", "investigation"
    if "?" in text or "what" in text or "why" in text:
